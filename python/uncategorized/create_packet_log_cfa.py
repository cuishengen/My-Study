...
def dumpPacket(fout, timestamp, type, data):
	...
	fout.write(arrayForNet32(length))
	print(type)
	fout.write(arrayForNet32(type_to_flag(type)))
	fout.write(arrayForNet32(0))
	fout.write(arrayForNet32(tv_sec))
	fout.write(arrayForNet32(tv_usec))
	fout.write(bytearray([type_to_hci(type)]))
	fout.write(data)
...
infile = sys.argv[1]
outfile = os.path.splitext(infile)[0] + ".cfa"
if len(sys.argv) > 2:
	outfile = sys.argv[2]
...
with open (outfile, 'wb') as fout:
	with open (infile, 'rt') as fin:
		packet_counter = 0
		line_conter = 0
		fout.write(b"btsnoop\x00\x00\x00\x00\x01\x00\x00\x03\xea")
		for line in fin:
			try:
				line_conter += 1
				timestamp = None
				...
				rest = chop(line,'CMD => ')
				if rest:
					handleHexPacket(fout, timestamp, TYPE_OUT_CMD, rest)
					continue
				rest = chop(line,'EVT <= ')
				if rest:
					handleHexPacket(fout, timestamp, TYPE_IN_EVT, rest)
					continue
				rest = chop(line,'ACL => ')
				if rest:
					handleHexPacket(fout, timestamp, TYPE_OUT_ACL, rest)
					continue
				rest = chop(line,'ACL <= ')
				if rest:
					handleHexPacket(fout, timestamp, TYPE_IN_ACL, rest)
					continue
				rest = chop(line,'SCO => ')
				if rest:
